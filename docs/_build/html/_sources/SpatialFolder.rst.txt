...
#. The spatial file contains multiple items of interest:
    ...
    * The *tissue_positions_list.csv* contains the following information, specified in column order, starting from the leftmost:

        * The unique barcode corresponds to a specific tixel.
        ...
        * The row this tixel is found in, is indexed starting from 0.

        * The column the tixel is found in, is indexed starting from 0.
...
To visualize both the number of UMIs and the number of raw counts within a tixel, a user must generate a file titled
...
From here, navigatw to under the **Visualization** tab, the user has three options to choose what they want the browser
to display.
    ...
    * **Count:** This setting colors each tixel according to a log transform of the total number of counts mapped to that tixel.
    ...
    While on any one of these settings, the user can click and flip the on or off tissue designation of a tixel.
